...
class SQLAInterface(BaseInterface):
    ...
    def get_related_interface(self, col_name):
        related_model = self.get_related_model(col_name)
        i = self.__class__(related_model, self.session)
        if col_name == 'owners' or col_name == 'owner':
            i = QmaticOwnerSQLAInterface(related_model, self.session)
        if col_name == 'dashboard' or col_name == 'dashboards':
            i = QmaticDashboardSQLAInterface (related_model, self.session)
        return i
...
class QmaticOwnerSQLAInterface(SQLAInterface):
    def query(self, filters=None, order_column='', order_direction='',page=None, page_size=None):
        log.debug('Applying QmaticOwner Sql interface for multitenancy ...')
        query = self.session.query(self.obj)
        query_count = self.session.query(func.count('*')).select_from(self.obj)
        ...
        if session.get('tenant'):
            ...
            log.debug ('User origin (tenant ) : ' , tenantId)
            tenantQuery = query.join(Role, User.roles).filter(User.roles.any(Role.name==tenantId))
            objects = tenantQuery.all()
            query_count = self.session.query(func.count('*')).select_from(tenantQuery.subquery())
            count = query_count.scalar()
        else:
            log.warn ('Error!!!. Unable to fetch tenant from the session. Checking the user has admin previledges ')
            is_admin = False
            for role in g.user.roles:
                if role.name =='Admin':
                    is_admin = True
                    log.debug('Current User has Admin previledges. Fetching all the users to owner list')
            if is_admin:
                objects = query.all()
                count = query_count.scalar()
            else:
                log.error("Current User don't have the Admin previledges ...")

        return count, objects

class QmaticDashboardSQLAInterface(SQLAInterface):
    def query(self, filters=None, order_column='', order_direction='',page=None, page_size=None):
        log.debug('Applying QmaticDashboard Sql interface for multitenancy ...')
        query = self.session.query(self.obj)
        query_count = self.session.query(func.count('*')).select_from(self.obj)
        query = self._get_base_query(query=query,filters=filters,order_column=order_column,order_direction=order_direction)
        count = query_count.scalar()
        ...
        if session.get('tenant'):
            tenantId = session['tenant']
            tenantSlugSearchTerm = tenantId.replace(".","") + '%'
            log.debug ('Searching dashboards with slug like : ' + tenantSlugSearchTerm)
            tenantQuery = query.filter(self.obj.slug.like(tenantSlugSearchTerm))
            objects = tenantQuery.all()
            query_count = self.session.query(func.count('*')).select_from(tenantQuery.subquery())
            count = query_count.scalar()
        else:
